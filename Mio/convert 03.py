...
# + Kein Verblassen (Belichtungs-Gain auf Luminanz)
# + Leichte Schärfung (Unsharp Mask auf Luminanz, farbtreu)
...
from scipy.ndimage import gaussian_filter  # für Unsharp Mask
...
# === Belichtungs‑Gain auf Luminanz (ohne Farbänderung) ===
def exposure_gain_on_luma(rgb, target_p99=0.85):
    """
    Skaliert das Bild global so, dass das 99. Perzentil der Luminanz ~ target_p99 ist.
    Gain ist global (ein Faktor für alle Kanäle) -> keine Farbverschiebung.
    """
...
# === Unsharp Mask nur auf Luminanz (Farben bleiben gleich) ===
def sharpen_luminance(rgb, radius=1.0, amount=0.6):
    """
    Schärft nur die Luminanz (Rec.709), skaliert danach die RGBs pro Pixel,
    sodass die Chromatik (Farben) erhalten bleibt.
    """
    rgb = np.clip(rgb, 0.0, 1.0)
    # Luma aus aktuellem RGB (wir behandeln Werte als Display-sRGB)
    Y = 0.2126 * rgb[..., 0] + 0.7152 * rgb[..., 1] + 0.0722 * rgb[..., 2]
    Y_blur = gaussian_filter(Y, sigma=radius)
    Y_high = Y - Y_blur
    Y_sharp = np.clip(Y + amount * Y_high, 0.0, 1.0)

    # pro Pixel Helligkeitsfaktor (verändert nur Helligkeit, nicht Farbe)
    denom = Y + 1e-8
    factor = Y_sharp / denom
...
def convert_hsi_to_rgb(mat_path, output_path, use_superres=False, target_bands=200):
    ...
    # 9) XYZ → sRGB (keine weitere sqrt/Gamma-Spielerei)
    rgb = XYZ_to_sRGB(xyz)
    rgb = np.clip(rgb, 0, 1)

    # 10) NICHT VERBLASSEN: globale Belichtungsanpassung auf Luma (Farben bleiben)
    rgb = exposure_gain_on_luma(rgb, target_p99=0.85)  # 0.85–0.90 je nach Geschmack

    # 11) LEICHT SCHÄRFER: Unsharp Mask nur auf Luminanz (Farben bleiben)
    rgb = sharpen_luminance(rgb, radius=1.0, amount=0.6)  # radius 0.8–1.2, amount 0.4–0.8

    # 12) Speichern
    os.makedirs(output_path, exist_ok=True)
    filename = os.path.splitext(os.path.basename(mat_path))[0]
    save_path = os.path.join(output_path, f"{filename}03_rgb.png")
